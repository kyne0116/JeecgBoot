...
5. 监控自动化处理：模块管理、前端迁移、SQL 执行、权限授权
   - **菜单权限SQL执行增强**：支持多路径查找策略，在正确模块路径未找到SQL文件时，自动在备用路径(jeecg-module-us等)中查找包含'menu'关键字且与业务实体相关的SQL文件
...
5. **结果反馈与报告**：
   ...
   - **反馈格式要求**：AI 执行任务反馈时，必须按照以下顺序显示：
     1. **执行状态汇总**：显示所有工作流步骤（通常为32个步骤）的详细状态，包括 ✅ PASS、❌ FAIL、⏭️ SKIP、🔄 RUNNING 四种状态
...
6. 最终必须生成结构化的执行报告，严格按照反馈顺序：执行状态汇总（显示所有32个步骤状态） → 生成的核心文件 → Maven 编译提醒（如果 Pass） → 总体执行结果（最后一行）
