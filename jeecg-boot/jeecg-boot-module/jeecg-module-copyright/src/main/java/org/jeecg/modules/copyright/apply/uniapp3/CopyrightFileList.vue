...
layout: 'default',
style: {
navigationBarTitleText: '软著文件记录',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="软著文件记录"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CopyrightFileData';
defineOptions({
  name: 'CopyrightFileList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/apply/copyrightFile/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/apply/copyrightFile/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
