...
<template>
  <PageLayout navTitle="软著申请配置表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CopyrightConfigData';
defineOptions({
  name: 'CopyrightConfigList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/apply/copyrightConfig/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/apply/copyrightConfig/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
