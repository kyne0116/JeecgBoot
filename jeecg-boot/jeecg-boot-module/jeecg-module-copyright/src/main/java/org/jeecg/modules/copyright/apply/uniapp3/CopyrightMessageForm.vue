...
navigationBarTitleText: '软著申请聊天记录',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['sequenceNo']"
                   :label="get4Label('消息序号')"
                   name='sequenceNo'
                   prop='sequenceNo'
                   placeholder="请选择消息序号"
                   inputMode="numeric"
...
const dataId = ref('')
const backRouteName = ref('CopyrightMessageList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/apply/copyrightMessage/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/apply/copyrightMessage/edit':'/apply/copyrightMessage/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
