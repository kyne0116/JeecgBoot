...
navigationBarTitleText: '软著申请AI日志表',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['agentName']"
                  :label="get4Label('Agent名称')"
                  name='agentName'
                  prop='agentName'
                  placeholder="请选择Agent名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['agentType']"
                  :label="get4Label('Agent类型')"
                  name='agentType'
                  prop='agentType'
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['executionPhase']"
                  :label="get4Label('执行阶段')"
                  name='executionPhase'
                  prop='executionPhase'
                  placeholder="请选择执行阶段"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['durationMs']"
                   :label="get4Label('执行时长(毫秒)')"
                   name='durationMs'
                   prop='durationMs'
                   placeholder="请选择执行时长(毫秒)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('输入参数JSON')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['inputParams']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('输出结果JSON')"
                 labelWidth="100px"
                 type="textarea"
                 name='outputResult'
                 prop='outputResult'
                 clearable
                 :maxlength="300"
                v-model="myFormData['outputResult']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['modelName']"
                  :label="get4Label('使用的模型名称')"
                  name='modelName'
                  prop='modelName'
                  placeholder="请选择使用的模型名称"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['totalTokens']"
                   :label="get4Label('总Token消耗')"
                   name='totalTokens'
                   prop='totalTokens'
                   placeholder="请选择总Token消耗"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['promptTokens']"
                   :label="get4Label('Prompt Token数')"
                   name='promptTokens'
                   prop='promptTokens'
                   placeholder="请选择Prompt Token数"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['completionTokens']"
                   :label="get4Label('完成Token数')"
                   name='completionTokens'
                   prop='completionTokens'
                   placeholder="请选择完成Token数"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
...
const dataId = ref('')
const backRouteName = ref('CopyrightAgentLogList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/log/copyrightAgentLog/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/log/copyrightAgentLog/edit':'/log/copyrightAgentLog/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
