...
import {columns} from './CopyrightSessionData';
defineOptions({
  name: 'CopyrightSessionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/apply/copyrightSession/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/apply/copyrightSession/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
